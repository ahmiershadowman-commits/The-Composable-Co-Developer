...
PRIMARY_CONTEXT_KEY = "content"
FAMILY = FamilyType.CONDUIT
PIPELINE_CHOICES = ['documentation', 'handoff_synthesis', 'professional_writing', 'scholarly_writing']
...
def run_pipeline(
    ...
    context = {
        PRIMARY_CONTEXT_KEY: primary_value or f"Conduit execution via {pipeline_id}",
    ...
    if FAMILY == FamilyType.FORENSICS:
        context["scope"] = {
            "description": primary_value or f"Conduit execution via {pipeline_id}",
            "boundaries": [],
        }
    if approval_json:
...
def main():
    ...
    parser.add_argument("--content", default="", help="Primary pipeline input")
