...
PRIMARY_CONTEXT_KEY = "question"
FAMILY = FamilyType.INQUIRY
PIPELINE_CHOICES = ['research', 'hypothesis_generation', 'data_analysis', 'formalization', 'mathematics', 'prompt_order_optimization', 'human_hint_integration']
...
def run_pipeline(
    ...
    print(f"Question: {primary_value}")
    ...
    context = {
        PRIMARY_CONTEXT_KEY: primary_value or f"Inquiry execution via {pipeline_id}",
    ...
    if FAMILY == FamilyType.FORENSICS:
        context["scope"] = {
            "description": primary_value or f"Inquiry execution via {pipeline_id}",
            "boundaries": [],
        }
    if approval_json:
...
def main():
    ...
    parser.add_argument("--question", default="", help="Primary pipeline input")
