...
Do not let Inquiry continue on untrusted state surfaces — reroute to Forensics. Do not use `hypothesis_generation` as a substitute for `research` when evidence exists but hasn't been collected. Do not promote experimental pipelines to default use; run them only with explicit approval and evidence.
